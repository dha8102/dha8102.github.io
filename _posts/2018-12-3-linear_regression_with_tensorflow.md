...
우리는 텐서플로우로 하여금 많은 x와 y 세트를 집어넣어 $H(x^{(i)})를 구해낼 것이다. 최소가 될 때  $H(x^{(i)})$(기대값)과 $y^{(i)})^{2}$(실제값)의 차이는 0에 가깝게 다가갈 것이다.
...
파란색의 var1, 노란색이 var2이다. 대략 6000번의 epoch를 거치면서 각각 2.0과 0.5로 수렴하여 간다.
