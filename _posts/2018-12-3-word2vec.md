...
<a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;u_{o}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\inline&space;u_{o}" title="u_{o}" /></a>는 주변단어들의 벡터, <<a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;v_{c}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\inline&space;v_{c}" title="v_{c}" /></a>는 중심단어의 벡터, <a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;u_{w}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\inline&space;u_{w}" title="u_{w}" /></a>는 말뭉치 내의 모든 단어를 나타낸다. (V의 범위는 설정하기 나름이다 -> hyperparameter) 이 값을 최대한 크게 해야 모델로부터 생성되는 벡터들의 매핑이 보다 정확하게 된다. 이 값을 크게 하려면 분자를 최대화, 분모를 최소화 시켜야 하는데, 
...
최종 결과 식에서 <a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;u_{o}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\inline&space;u_{o}" title="u_{o}" /></a>는 바꿀 수 없는 값이고(이미 observed된 것), 바꿀 수 있는 것은 minus 부호 뒤의 값들인데, 이들은 V의 범위를 조정함으로써 바꿀 수 있다. 이 식의 값을 딥 러닝 기법을 통해 최적화 하는 것이 word2vec 모델에서 쓰인 수학적 기법이라 할 수 있겠다.